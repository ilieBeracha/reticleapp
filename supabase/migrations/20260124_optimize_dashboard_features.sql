-- ============================================================================
-- OPTIMIZE DASHBOARD: Add is_timed + fix created_at in session_features
-- ============================================================================
-- Adds is_timed boolean for filter support, and ensures created_at matches
-- session.started_at for proper time-based queries from session_features.
-- ============================================================================

-- 1. Add is_timed column
ALTER TABLE public.session_features
  ADD COLUMN IF NOT EXISTS is_timed boolean DEFAULT false;

-- 2. Update compute_session_features to populate is_timed and set created_at
...
'Computes and stores derived features for a session including weather and is_timed. Call after session completion. Idempotent (upserts).';

-- 3. Backfill existing rows: set is_timed from drill_templates/custom_drill_config
UPDATE public.session_features sf
SET is_timed = COALESCE(
  (
    SELECT
      d.time_limit_seconds IS NOT NULL
      OR (s.custom_drill_config->>'time_limit_seconds') IS NOT NULL
    FROM sessions s
    LEFT JOIN drill_templates d ON d.id = s.drill_template_id
    WHERE s.id = sf.session_id
  ),
  false
);

-- 4. Backfill created_at from sessions.started_at
UPDATE public.session_features sf
SET created_at = s.started_at
FROM sessions s
WHERE s.id = sf.session_id
  AND sf.created_at != s.started_at;
