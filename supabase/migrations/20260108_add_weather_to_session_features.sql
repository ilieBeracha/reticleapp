-- ============================================================================
-- ADD WEATHER TO SESSION FEATURES
-- ============================================================================
-- Adds weather columns to session_features for insight analysis.
-- Weather data is extracted from sessions.weather JSONB column.
-- ============================================================================

-- 1. Add weather columns to session_features
ALTER TABLE public.session_features 
  ...
  ADD COLUMN IF NOT EXISTS weather_wind_speed_mps numeric,
  ADD COLUMN IF NOT EXISTS weather_wind_bearing integer,
  ADD COLUMN IF NOT EXISTS weather_condition text,
  ADD COLUMN IF NOT EXISTS weather_wind_impact text,           -- 'negligible' | 'minor' | 'moderate' | 'significant' | 'severe'
  ADD COLUMN IF NOT EXISTS weather_condition_severity text;    -- 'ideal' | 'good' | 'moderate' | 'poor' | 'severe'

-- 2. Index for weather-based queries
CREATE INDEX IF NOT EXISTS idx_session_features_weather_condition 
  ON public.session_features (weather_condition) 
  WHERE has_weather = true;

CREATE INDEX IF NOT EXISTS idx_session_features_weather_wind 
  ON public.session_features (weather_wind_impact) 
  WHERE has_weather = true;

-- 3. Update compute_session_features to extract weather
...
'Computes and stores derived features for a session including weather. Call after session completion. Idempotent (upserts).';
