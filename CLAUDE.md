# Scope Stats — Product Specification

**Purpose**: Complete product specification for the Reticle sniper training application. This document describes the MVP features, user flows, and UI/UX requirements.
...
* **commander** (scoped by level and entity): `{ role: 'commander', scope_level: 'unit'|'team'|'squad', scope_id: UUID }`
...
### Dashboard — Scoped (Commander/Instructor)
...
**Acceptance (MVP)**: Finish persists; Review shows computed metrics; drafts autosave; offline capture supported.
...
## Data Mapping (UI ↔️ Tables/Stores)

**Hierarchy**

* `organizations` (id, name, org_type, parent_id, root_id, path, depth)
* `org_memberships` (user_id, org_id, role: 'commander'|'instructor'|'member'|'viewer')

**Trainings & Sessions**

* `trainings` (id, org_id, title, schedule, location, goals, status)
* `session_stats` (id, user_id, org_id?, training_id?, type: 'paper'|'steel', position, distance_m, wind_speed_kmh, wind_dir_deg, weapon_id, sight_id, equipment_id?, created_at)
* `target_stats` (session_id, shots, hits, eliminated bool)
* `group_scores` (session_id, participant_id, cm_dispersion, shooting_position)
* `session_participants` (session_id, user_id, role)
* Policy: Only sessions with `training_id` are counted in org analytics

**Loadouts & Equipment**

* `weapons` (id, model_id, org_id, owner_user_id, serial_number, modifications)
* `weapon_models` (id, name, manufacturer, caliber, typical_muzzle_velocity)
* `sights` (id, model_id, org_id, owner_user_id, serial_number, calibration)
* `sight_models` (id, name, type, magnification_range, reticle_type)

**Insights & Analytics**

* AI insights stored/computed on-demand
* Commander rules [Later]
