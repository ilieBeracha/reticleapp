...
function ParallaxScrollContent({
  ...
  return (
    ...
    >
        ...
        <View style={styles.statusBar}>
          ...
          {isOngoing && isWatchConnected && (
            <TouchableOpacity
              ...
              onPress={() => onCapturePreferenceSelect(
                trainingWatchPreference ? 'phone' : 'watch',
                trainingSensitivity
              )}
              activeOpacity={0.7}
            >
          ...
          {/* Custom Session - quick add */}
          {isOngoing && !!userWeapon && (
            <TouchableOpacity
              style={[styles.statusItem, { backgroundColor: colors.primary }]}
              onPress={() => setShowCustomSession(true)}
              disabled={customSessionStarting}
              activeOpacity={0.8}
            >
      ...
      {showTabs && (
        <View style={[styles.tabBar, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          >
            ...
            <Text style={[styles.tabText, { color: activeTab === 'squad' ? colors.primary : colors.textMuted }]}>
              Squad
            </Text>
            {teamSessions.length > 0 && (
              <View style={[styles.tabBadge, { backgroundColor: colors.green }]}>
                <Text style={styles.tabBadgeText}>{new Set(teamSessions.map((s: any) => s.user_id)).size}</Text>
              </View>
            )}
          </TouchableOpacity>
        </View>
          ...
          {isFinished &&
            ...
            (() => {
              ...
              const totalShots = completedSessions.reduce(
                (sum: number, s: any) => sum + (s.stats?.shots_fired || 0),
                0
              );
                ...
                <View style={[styles.roundSummaryCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                  ...
                  {insight && (
                    <View style={[styles.roundInsight, { backgroundColor: colors.primary + '08' }]}>
                      <Text style={[styles.roundInsightText, { color: colors.primary }]}>{insight}</Text>
                    </View>
                  )}
                </View>
              );
      ...
      {/* Custom Session Modal - controlled by SessionSetupCard */}
      <CustomSessionCard
        visible={showCustomSession}
        onClose={() => setShowCustomSession(false)}
        hideTrigger
        onStartSession={async (config) => {
          await onStartCustomSession(config);
          setShowCustomSession(false);
        }}
        hasWeapon={!!userWeapon}
        isStarting={customSessionStarting}
...
export default function TrainingDetailScreen() {
  ...
  const [customSessionStarting, setCustomSessionStarting] = useState(false);
  ...
  useEffect(() => {
    if (allDrillsCompleted && canManageTraining && !autoFinishTriggeredRef.current && !actionLoading) {
      autoFinishTriggeredRef.current = true;

      // Show a brief confirmation before auto-finishing
      Alert.alert('All Drills Completed! 🎯', 'Great work! Would you like to complete this training now?', [
        {
          text: 'Stay Open',
          style: 'cancel',
          onPress: () => {
  ...
  const shouldLoadTeamData =
    !!training?.id && canManageTraining && (training?.status === 'ongoing' || training?.status === 'finished');
    ...
    async (drill: any) => {
      ...
      if (isWatchConnected && trainingWatchPreference === null) {
        ...
        Alert.alert(
          ...
          'Select how to record your session using the card above the drills.',
  ...
  const handleStartCustomSession = useCallback(
    async (config: CustomSessionConfig) => {
      if (!userWeapon || !training?.id) return;

      setCustomSessionStarting(true);
      try {
        const sessionWeather = toSessionWeatherData(openWeather, 'openweathermap');
        ...
        const sessionConfig: BaseSessionConfig = {
          ...
          drill_id: null,
          drill_config: {
            name: `${config.purpose === 'grouping' ? 'Grouping' : 'Engagement'} ${config.distance}m`,
            ...
            // Include detection sensitivity when using watch mode
            ...(config.watchControlled && config.detectionSensitivity && {
              detection_sensitivity: config.detectionSensitivity,
            }),
          },
          session_mode: 'solo',
          watch_controlled: config.watchControlled,
          start_as_pending: false,
        };

        const newSession = await createSession(sessionConfig);
        await loadSessions();
        ...
        setCustomSessionStarting(false);
      }
    },
    [userWeapon, training?.id, training?.team_id, openWeather, loadSessions]
  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ParallaxScrollContent
        ...
        customSessionStarting={customSessionStarting}
      />

      {/* Bottom Actions - Only show when action is needed */}
      {((isPlanned && canManageTraining) || (isOngoing && canManageTraining && allDrillsCompleted)) && (
            ...
            >
              {actionLoading ? (
                ...
                <>
                  ...
                  <Text style={[styles.mainActionBtnText, { color: '#fff' }]}>Complete Training</Text>
                </>
              )}
            </TouchableOpacity>
          )}
