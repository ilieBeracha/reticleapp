...
const TeamCard = React.memo(function TeamCard({
  ...
  const hasCommander = memberCount > 0; // Simplified check
...
const ManageScreen = React.memo(function ManageScreen() {
  ...
  const canInvite = useMemo(() => {
    // Owner and Admin can invite anyone
    if (orgRole === 'owner' || orgRole === 'admin') return true;
    // Team commanders can invite to their team
  ...
  // NOTE: Double arrow function required because setState(fn) calls fn as updater
  // So () => callback stores `callback`, but () => { doStuff() } executes immediately
  useEffect(() => {
