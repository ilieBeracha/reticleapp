...
**Personal Context** (`orgId = null`):
```typescript
// Returns only user's personal data
const sessions = await getSessionsService(userId, null)
```

**Organization Context** (`orgId = UUID`):
```typescript
// Returns all data within organization
...
- **`.cursorrules`** - Complete architecture rules and coding patterns
- **`CLAUDE.md`** - Product specifications and user flows
- **`.github/copilot-instructions.md`** - Quick reference for AI assistants
- **`.coderabbit.yaml`** - Path-based code review configuration
...
Service layer functions are documented with JSDoc comments. Example:
...
2. **Named Exports**: Use `export function Component()` (not default exports)
3. **Service Layer**: ALL database operations go through services
4. **Colocated Styles**: No separate `.styles.ts` files
5. **Error Handling**: Use custom error types, never silent failures
6. **Testing**: Test both personal and organization contexts

### Pull Request Process

1. Create a feature branch from `dev-main`
2. Follow the architecture patterns in `.cursorrules`
3. Write clear commit messages
4. Ensure TypeScript compiles without errors
5. Test on both iOS and Android (if UI changes)
6. Submit PR with description of changes

### Code Review

All PRs are automatically reviewed by:
- **CodeRabbit AI** - Architectural and code quality checks
- **Human reviewers** - Functional and design review
...
- ✅ JWT tokens automatically injected (never passed as parameters)
...
Please report security vulnerabilities to: **security@reticle.app**
...
Built with ❤️ by the Reticle team.

For questions or support, contact: **support@reticle.app**
...
### Current (v1.0 - MVP)
- ✅ Personal session capture (paper + steel)
- ✅ Organizational hierarchy and trainings
- ✅ Basic analytics and performance tracking
- ✅ Offline-first operation
- ✅ Role-based access control

### Planned (v1.1)
- 🔄 AI-powered training insights (RAG-based)
- 🔄 Commander rules and compliance tracking
- 🔄 PDF export for analytics
- 🔄 Garmin integration
- 🔄 Advanced target detection (computer vision)
