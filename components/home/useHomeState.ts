...
export function useHomeState({ sessions, upcomingTrainings, hasTeams }: UseHomeStateParams): HomeState {
  return useMemo(() => {
    ...
    const lastSession = completedSessions[0] ? mapSessionToHomeSession(completedSessions[0]) : null;
    ...
    return {
      ...
      weeklyStats: {
        ...
        avgAccuracy:
          weeklyStats.accuracyCount > 0 ? Math.round(weeklyStats.accuracySum / weeklyStats.accuracyCount) : 0,
      },
      hasTeams,
    };
