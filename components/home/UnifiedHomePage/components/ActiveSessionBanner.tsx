 ...
 *
 * Subtle notification bar for active solo session.
 * Mirrors ActiveTrainingBanner design: compact, professional.
 */
...
export function ActiveSessionBanner({ session, onPress, colors }: ActiveSessionBannerProps) {
  ...
  return (
    ...
    >
      ...
      {/* Continue Button */}
      <TouchableOpacity
        style={[s.continueBtn, { backgroundColor: colors.text }]}
        onPress={onPress}
        activeOpacity={0.7}
      >
        <Play size={12} color={colors.background} strokeWidth={2.5} fill={colors.background} />
        <Text style={[s.continueText, { color: colors.background }]}>Continue</Text>
      </TouchableOpacity>
    </Animated.View>
  );
}
