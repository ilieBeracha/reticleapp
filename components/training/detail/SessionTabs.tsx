  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <TouchableOpacity
        style={[
          styles.tab,
          activeTab === 'grouping' && [styles.activeTab, { backgroundColor: colors.primary }],
        ]}
        onPress={() => handleTabPress('grouping')}
        activeOpacity={0.7}
      >
        <Crosshair size={16} color={activeTab === 'grouping' ? '#fff' : colors.textMuted} />
        <Text style={[styles.tabText, { color: activeTab === 'grouping' ? '#fff' : colors.text }]}>
          Grouping
        </Text>
        {groupingCount > 0 && (
          <View
          ...
          >
            <Text
              style={[
                styles.badgeText,
                { color: activeTab === 'grouping' ? '#fff' : colors.textMuted },
              ]}
            >
              {groupingCount}
            </Text>
      ...
      <TouchableOpacity
        style={[
          styles.tab,
          activeTab === 'engagement' && [styles.activeTab, { backgroundColor: colors.orange }],
        ]}
        onPress={() => handleTabPress('engagement')}
        activeOpacity={0.7}
      >
        <Target size={16} color={activeTab === 'engagement' ? '#fff' : colors.textMuted} />
        <Text style={[styles.tabText, { color: activeTab === 'engagement' ? '#fff' : colors.text }]}>
          Engagement
        </Text>
        {engagementCount > 0 && (
          <View
          ...
          >
            <Text
              style={[
                styles.badgeText,
                { color: activeTab === 'engagement' ? '#fff' : colors.textMuted },
              ]}
            >
              {engagementCount}
            </Text>
