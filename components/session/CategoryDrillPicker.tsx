...
import { type CategoryDrill, type DrillType, groupDrillsByType } from '@/constants/categoryDrills';
...
export function CategoryDrillPicker({
  ...
  const drillTypes = filterTypes?.length ? allDrillTypes.filter((type) => filterTypes.includes(type)) : allDrillTypes;
      ...
      {drillTypes.map((type) => {
        ...
        return (
          <View key={type} style={styles.section}>
            <Text style={[styles.sectionLabel, { color: colors.textMuted }]}>{DRILL_TYPE_LABELS[type]}</Text>
      ...
      <View style={styles.cardMain}>
        ...
        <View style={styles.cardMeta}>
          ...
          <View style={styles.cardRight}>
            ...
            <Text style={[styles.diffText, { color: diffColor }]}>{drill.difficulty.charAt(0).toUpperCase()}</Text>
          </View>
        </View>
      </View>
    </TouchableOpacity>
