...
interface MemberPreviewSheetProps {
  member: WorkspaceMemberWithTeams | null;
  onEditRole?: () => void;
  onRemoveMember?: () => void;
  onViewActivity?: () => void;
}

export const MemberPreviewSheet = forwardRef<BaseDetachedBottomSheetRef, MemberPreviewSheetProps>(
  ({ member, onEditRole, onRemoveMember, onViewActivity }, ref) => {
          ...
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            onPress: () => {
              if (onRemoveMember) {
                ...
                // Placeholder for future implementation
                Alert.alert('Coming Soon', 'Member removal will be available soon');
              }
            },
          },
        ]
        ...
        <View style={styles.container}>
          ...
          <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <View style={styles.actionsList}>
              ...
              >
                <View style={styles.actionContent}>
                  <Ionicons
                    ...
                    size={20}
                    color={isOwner ? colors.textMuted : colors.text}
                    style={{ opacity: isOwner ? 0.4 : 1 }}
                  />
                  <Text
                    style={[
                      styles.actionText,
                      { color: isOwner ? colors.textMuted : colors.text, opacity: isOwner ? 0.4 : 1 },
                    ]}
                  >
                    Edit Role
                  </Text>
              ...
              {/* Remove Member - disabled for owner */}
              <TouchableOpacity
              ...
              >
                <View style={styles.actionContent}>
                  <Ionicons
                    name="person-remove"
                    size={20}
                    color={isOwner ? colors.textMuted : '#ef4444'}
                    style={{ opacity: isOwner ? 0.4 : 1 }}
                  />
                  <Text
                    style={[
                      styles.actionText,
                      { color: isOwner ? colors.textMuted : '#ef4444', opacity: isOwner ? 0.4 : 1 },
                    ]}
                  >
                    Remove Member
                  </Text>
          ...
          {isOwner && (
            <Text style={[styles.ownerNote, { color: colors.textMuted }]}>
...
MemberPreviewSheet.displayName = 'MemberPreviewSheet';
