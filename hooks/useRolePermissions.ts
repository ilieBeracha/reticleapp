...
export function useRolePermissions(): UseRolePermissionsReturn {
  const { activeProfile, myRole } = useProfile();
  
  const orgRole = myRole as OrgRole | null;
  // Get team role directly from profile (simplified architecture)
  const teamRole = activeProfile?.team_role as TeamRole | undefined;
